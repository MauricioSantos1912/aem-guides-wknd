...
(function(window, document, Granite, $) {
    ...
    function validate(children, sanitizedChildren) {
        var childrenArr = Array.from(children);
        var sanitizedChildrenArr = Array.from(sanitizedChildren);
        childrenArr.forEach(function(child, index) {
            var sanitizedChild = sanitizedChildrenArr[index];
            if (sanitizedChild && child.isEqualNode(sanitizedChild)) {
                child.remove();
                sanitizedChild.remove();
            } else if (sanitizedChild) {
                validate(child.children, sanitizedChild.children);
            }
        });
    }

    function validateDiff(html, sanitizedHtml) {
        var doc = (new DOMParser()).parseFromString(html, "text/html");
        var sanitizedDoc = (new DOMParser()).parseFromString(sanitizedHtml, "text/html");

        if (doc.isEqualNode(sanitizedDoc)) {
            return;
        }

        validate(doc.children, sanitizedDoc.children);

        if (!doc.isEqualNode(sanitizedDoc)) {
            // eslint-disable-next-line no-console
            console.warn("Below tags are being removed while sanitization.\n", doc);
        }
    }
    ...
    function getConfig(config) {
        var defaultConfig = {
            ...
            CUSTOM_ELEMENT_HANDLING: {
                tagNameCheck: function() {
                    return true;
                },
                attributeNameCheck: function() {
                    return true;
                },
        ...
        // @todo add code to take config into account, add change Dompurify configuration per sanitization.
        return defaultConfig;
    }

    Granite.UI.Foundation.XSS = Granite.UI.Foundation.XSS || (function() {
        return {
            validUrlWithPrefix: function(href, prefix) {
                return this.validUrlWithPrefixes(href, [ prefix ]);
            },

            validUrlWithPrefixes: function(href, prefixes) {
                var isValid = false;
                ...
                if (this.isSameOrigin(href)) {
                    ...
                    for (var i = 0; i < prefixes.length; i++) {
                        ...
                        if (url.pathname.startsWith(prefix) ||
                            url.pathname.startsWith(Granite.HTTP.getContextPath() + prefix)) {
                            isValid = true;
                            break;
                        }
                    }
            ...
            sanitizeHtml: function(html, config) {
                var sanitizedHtml = DOMPurify.sanitize(html, getConfig(config));
                // @todo remove diff, once majority securituy bug has been resolved.
                validateDiff(html, sanitizedHtml);
                return sanitizedHtml;
            },

            isSameOrigin: function(urlToTest) {
                try {
            ...
            isNullOrigin: function(urlToTest) {
                try {
                    return new URL(urlToTest, document.baseURI).origin === null;
                } catch (e) {
                    return true;
                }
            }
...
(function(window, document, Granite, $) {
    ...
    Granite.UI.Foundation.Utils = Granite.UI.Foundation.Utils || (function() {
        ...
        return {
            ...
            // deprecated use Granite.UI.Foundation.XSS.sanitizeHtml
            sanitizeHtml: function(html, config) {
                // eslint-disable-next-line no-console, max-len
                console.warn("`Granite.UI.Foundation.Utils.sanitizeHtml` is deprecated, use `Granite.UI.Foundation.XSS.sanitizeHtml` instead");
                return Granite.UI.Foundation.XSS.sanitizeHtml(html, config);
            }
        };
    })();

    Granite.UI.Foundation.Utils.XSS = Granite.UI.Foundation.Utils.XSS || (function() {
        // eslint-disable-next-line no-console
        console.warn("`Granite.UI.Foundation.Utils.XSS` APIs are deprecated, use `Granite.UI.Foundation.XSS` instead");
        return Granite.UI.Foundation.XSS;
    })();
})(window, document, Granite, Granite.$);
            ...
            return {
                ...
                notify: function(title, message, type, notifyTime) {
                    ...
                    if (Coral.Toast) {
                        ...
                        el.variant = type;
                        el.content.innerHTML = Granite.UI.Foundation.XSS.sanitizeHtml(message || title);
                        ...
                        el.header.textContent = title;
                        el.content.innerHTML = Granite.UI.Foundation.XSS.sanitizeHtml(message || "");
        ...
        validate: function(el) {
            ...
            for (var i = 0, ln = v.length; i < ln; i++) {
                if (NON_VALID_CHARS.indexOf(v[i]) >= 0 || v.charCodeAt(i) > 127) {
                    ...
                    return Granite.I18n.get("Invalid character '{0}'. The input must use characters matching the following regular expression {1}.", [ v[i], _ALLOWED_FILENAME_CHARS ]);
                }
            }
        }
    });
...
(function(document, $, Granite) {
    ...
    document.addEventListener("click", function(e) {
        ...
        var sanitizedTarget = Granite.UI.Foundation.XSS.sanitizeHtml(target);
    ...
    $(document).on("click", ".foundation-collection-action", function(e) {
        ...
        if (itemEl) {
            ...
            if (config.target) {
                var sanitizedTarget = Granite.UI.Foundation.XSS.sanitizeHtml(config.target);
                collection = $(sanitizedTarget);
    ...
    $(window).adaptTo("foundation-registry").register("foundation.collection.action.action", {
        ...
        handler: function(name, el, config, collection, selections) {
            ...
            if (contentAPI) {
                ...
                if (Granite.Toggles.isEnabled("FT_GRANITE-46844")) {
                    contentAPI.pushState(config.data.data, config.data.title, url, true);
                } else {
...
(function(document, $) {
    ...
    $(document).on("foundation-selections-change", ".foundation-collection-masterdetail", function(e) {
        ...
        var detailCollection = $(Granite.UI.Foundation.XSS.sanitizeHtml(detailSelector));
...
(function(document, $) {
    ...
    function resolveToggleable(control) {
        ...
        if (src) {
            ...
            return $.ajax({
                ...
                var sanitizedEl = Granite.UI.Foundation.XSS.sanitizeHtml(html);
                var el = $(sanitizedEl)
                    .on("foundation-toggleable-hide", function(e) {
                        var target = $(e.target);
    ...
    $(document).on("click", ".foundation-toggleable-control", function(e) {
        ...
        if (control.hasClass("is-selected")) {
...
(function(window, $) {
    ...
    $(window).adaptTo("foundation-registry").register("foundation.form.response.ui.error", {
        ...
        handler: function(form, data, xhr, error, errorThrown) {
            ...
            message = Granite.UI.Foundation.XSS.sanitizeHtml(message);
            var ui = $(window).adaptTo("foundation-ui");
            ui.alert(title, message, "error");
        }
    });
...
(function(window, $) {
    ...
    function renderOutput(form, config, html) {
        var panel = $(Granite.UI.Foundation.XSS.sanitizeHtml(config.target));

        var contentAPI = panel.adaptTo("foundation-content");

        if (!contentAPI) {
            panel.html(Granite.UI.Foundation.XSS.sanitizeHtml(html));
            return;
        }
...
(function(window, $, URITemplate) {
    ...
    $(window).adaptTo("foundation-registry").register("foundation.form.response.ui.success", {
        ...
        handler: function(form, config, data, textStatus, xhr) {
            ...
            if (Granite.UI.Foundation.XSS.isSameOrigin(href)) {
                window.location = href;
            }
        }
    });
...
(function(window, $, Granite) {
    ...
    $(window).adaptTo("foundation-registry").register("foundation.form.response.ui.success", {
        ...
        handler: function(form, config, data, textStatus, xhr) {
            ...
            messenger.put(
                null,
                Granite.UI.Foundation.XSS.sanitizeHtml(
...
(function(window, $, Granite) {
    ...
    $(window).adaptTo("foundation-registry").register("foundation.form.response.ui.success", {
        ...
        handler: function(form, config, data, textStatus, xhr) {
            ...
            var collection = $(Granite.UI.Foundation.XSS.sanitizeHtml(config.target));
    ...
    $(window).adaptTo("foundation-registry").register("foundation.adapters", {
        ...
        adapter: function(el) {
            ...
            return {
                ...
                pushState: function(data, title, url, isDomPurifyReq) {
                    isDomPurifyReq = (isDomPurifyReq | false);
                ...
                replaceState: function(data, title, url, isDomPurifyReq) {
                    isDomPurifyReq = (isDomPurifyReq | false);
                ...
                go: function(url, replace, historyConfig, isDomPurifyReq) {
                    var self = this;
                    isDomPurifyReq = (isDomPurifyReq | false);
                    ...
                    return xhr
                        .then(function(html) {
                            html = isDomPurifyReq ? Granite.UI.Foundation.XSS.sanitizeHtml(html) : html;
                            if (replace) {
...
(function(document, $) {
    ...
    $(document).on("click", "a[data-foundation-content-history]", function(e) {
        ...
        if (info.replace) {
            api.replaceState(info.data, info.title, info.url, true);
        } else {
            api.pushState(info.data, info.title, info.url, true);
        }
    });
})(document, Granite.$);
    ...
    $(document).on("click", ".foundation-picker-control", function(e) {
        ...
        if (state.el) {
            ...
            $.ajax({
                ...
                var sanitizedHtml = Granite.Toggles.isEnabled("FT_GRANITE-45484")
                    ? Granite.UI.Foundation.XSS.sanitizeHtml(html) : html;
                return $(window).adaptTo("foundation-util-htmlparser").parse(sanitizedHtml);
            }).then(function(fragment) {
                return $(fragment).children()[0];
...
(function(document, Granite, $) {
    ...
    $(document).on("click", ".foundation-layout-control", function(e) {
        ...
        if (config.action === "switch") {
            // eslint-disable-next-line max-len
            var target = config.target ? $(Granite.UI.Foundation.XSS.sanitizeHtml(config.target)) : control.closest("[data-foundation-layout]");
    ...
    registry.register("foundation.adapters", {
        ...
        adapter: function(el) {
            ...
            var handleElementEscKey = function(event) {
                ...
                var triggerButton = popover.context._elementToFocusWhenHidden;
...
(function(window, document, Granite, jquery, Coral, URITemplate) {
    ...
    function handleQuickactions(state) {
        ...
        var overlay = function(e) {
            ...
            var anchorHTMLCollection = e.target.getElementsByTagName("a");
            for (var index = 0; index < anchorHTMLCollection.length; index++) {
                var element = anchorHTMLCollection.item(index);
                if (element.matches("a[is='coral-anchorbutton']")) {
...
(function(document, $) {
    ...
    function navigate(control) {
        ...
        $.ajax(src)
            .done(function(html) {
                var parser = $(window).adaptTo("foundation-util-htmlparser");
                var sanitizedHtml = Granite.UI.Foundation.XSS.sanitizeHtml(html);
...
(function(document, $) {
    ...
    $(document).on("foundation-contentloaded", function(e) {
        $(".granite-collection-create", e.target).each(function() {
            // eslint-disable-next-line max-len
            var sanitizedTarget = Granite.UI.Foundation.XSS.sanitizeHtml(this.dataset.foundationToggleableControlTarget);
