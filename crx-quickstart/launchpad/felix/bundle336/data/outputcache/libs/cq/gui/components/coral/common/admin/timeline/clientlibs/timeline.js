...
(function(document, $) {
    ...
    $(document).on("keyup", eventSelector, function(e) {
        if (e.keyCode === 13 || e.keyCode === 32 || e.keyCode === 27) { // Keycode 13 = Enter button, 32 = space, 27 = esc
            ...
            var target = $(e.target);
            var $this = $(this);
            var eventSelectorToExpand = $this.closest(".version-section-container");
            ...
            if (e.keyCode === 27 && $this.hasClass("is-active")) {
                ...
                deselectAll();
                eventSelectorToExpand.addClass("is-active");
                eventSelectorToExpand.children("section").attr("aria-expanded", true);
            }
        }
        // hide all toggleables (actions of workflow alerts or toolbar)
        $(toggleableSelector).attr("hidden", true);
        ...
        if ((paths.length === 0) || (paths.length === 1 && paths[0] === "")) {
            ...
            if (paths.length > 1) {
                ...
                if (isAvailable) {
                    ...
                    var link;
                    try {
                       link = new URL($(eventsSelector).data("url"));
                    } catch (er) {
                        link = null;
                    }
                    $.ajax({
                        url: link === null ? $(eventsSelector).data("url") : encodeURIComponent($(eventsSelector).data("url")),
                        data: {
                            "item": paths[0],
